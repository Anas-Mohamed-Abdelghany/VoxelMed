...
export default function Toolbar() {
  ...
  return (
    <div className="h-11 flex items-center justify-between px-3 bg-med-panel border-b border-med-border shrink-0 select-none z-30">
      {/* Left controls */}
      <div className="flex items-center gap-3">
        ...
        <div className="w-px h-5 bg-med-border mx-1" />

        <button
          onClick={() => fileRef.current?.click()}
          className="px-3 py-1 bg-med-accent hover:bg-med-accent-hover text-white text-xs font-semibold rounded shadow transition-colors flex items-center gap-1.5"
        >
          <span>📁</span>
          <span>Open File</span>
        </button>
        ...
        <button
          ...
          className={`px-2.5 py-1 text-xs border rounded flex items-center gap-1.5 transition-all font-medium disabled:opacity-30 ${
            isRestored
        ...
        >
          ...
          <span>{isRestored ? 'Motion Filter: Enabled' : 'Motion Filter: Disabled'}</span>
        </button>
      </div>

      {/* Right metadata */}
      {volumeInfo && (
        <div className="text-med-text-dim text-xs flex items-center gap-2 font-mono">
